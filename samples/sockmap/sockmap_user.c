			     ...
			     struct msg_stats *s)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		printf("%s:%i: sent %i/%i @ %i\n", __func__, i, sent, iov_length, 0);
		if (sent < 0) {
		...
		while (s->bytes_recvd < total_bytes) {
			...
			if (data_test) {
				...
				for (i = 0; i < msg.msg_iovlen; i++) {
					...
					for (j = 0; j < iov[i].iov_len && recv; j++) {
						if (d[j] != k++) {
							errno = -EIO;
							fprintf(stderr, "detected data corruption @iov[%i]:%i %02x != %02x, %02x ?= %02x\n",
			...
			struct sockmap_options *opt)
{
	...
	if (rxpid == 0) {
		if (opt->sendpage)
			iov_count = 1;
		err = msg_loop(rx_fd, iov_count, iov_buf, cnt, &s, false, opt->data_test);
		if (err)
			fprintf(stderr,
	...
	if (txpid == 0) {
		if (opt->sendpage)
			...
			err = msg_loop(c1, iov_count, iov_buf, cnt, &s, true, opt->data_test);
...
run:
	...
	if (tx_prog_fd) {
		...
		if (err) {
			fprintf(stderr, "ERROR: bpf_prog_attach (txmsg): %d (%s)\n",
				err, strerror(errno));
			return err;
		}
		...
		if (err) {
			fprintf(stderr, "ERROR: bpf_map_update_elem (txmsg):  %d (%s\n",
				err, strerror(errno));
			return err;
		}
		...
		if (txmsg_apply) {
			...
			if (err) {
				fprintf(stderr, "ERROR: bpf_map_update_elem (apply_bytes):  %d (%s\n",
					err, strerror(errno));
				return err;
			}
		}
