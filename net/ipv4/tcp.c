...
static void tcp_sk_redirect(struct sock *sk, struct sk_buff *skb, int *bpf)
{
	struct tcp_sock *tp = tcp_sk(sk);
	int err;
...
new_segment:
		...
		bpf = bpf ? bpf : BPF_CGROUP_RUN_PROG_TCP_SEND(sk, skb);
		if (bpf == SK_DROP) {
...
new_segment:
		...
		if (skb_availroom(skb) > 0) {
			...
			if (err)
				goto do_fault;
		} else if (!uarg || !uarg->zerocopy || bpf_cgroup_tcp_sendmsg_enabled(sk)) {
			bool merge = true;
