...
static int sk_psock_skb_ingress(struct sk_psock *psock, struct sk_buff *skb)
{
	...
	/* This is not only unlikely, but means we have built a skb
	 * that can not fit in a newly allocated sk_msg. This should
	 * not be possible if it happens we can only drop the data.
	 */
	if (unlikely(num_sge < 0)) {
		WARN_ONCE(1, "sk_psock: skb too large for sgvec\n");
		kfree(msg);
		return num_sge;
	}
